...
class PICPolicy:
    ...
    require_pic_for_impacts: Set[str] = field(
        default_factory=lambda: {"money", "privacy", "irreversible"}
    )

    # If a tool's impact is in this set, evidence verification must pass (when verify_evidence=True)
    require_evidence_for_impacts: Set[str] = field(
        default_factory=lambda: {"money", "privacy", "irreversible"}
    )

    def get_tool_impact(
        self, tool_name: str, proposal_impact: Optional[str] = None
    ) -> Optional[str]:
